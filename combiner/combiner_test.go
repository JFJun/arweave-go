...
func sendTransaction(hash string) *tx.Transaction {
	...
	switch hash {
	case "0xa":
		return createNewTestTransaction([]byte("0xa"), fmt.Sprintf("{\"data\": \"%s\", \"position\": %d}", utils.EncodeToBase64(data1), 0), []tx.Tag{tx.Tag{Name: arweave_go.BatchChunkerAppName, Value: string(tag1)}})
	case "0xb":
		return createNewTestTransaction([]byte("0xb"), fmt.Sprintf("{\"data\": \"%s\", \"position\": %d}", utils.EncodeToBase64(data2), 1), []tx.Tag{tx.Tag{Name: arweave_go.BatchChunkerAppName, Value: string(tag2)}})
	case "0xc":
		return createNewTestTransaction([]byte("0xc"), fmt.Sprintf("{\"data\": \"%s\", \"position\": %d}", utils.EncodeToBase64(data3), 2), []tx.Tag{tx.Tag{Name: arweave_go.BatchChunkerAppName, Value: string(tag3)}})
	}
	return nil
}
